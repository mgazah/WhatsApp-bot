/**
 * AutoSticker Command - Enable or disable auto-sticker conversion
 */

const database = require('../../database');

module.exports = {
  name: 'antiticker',
  aliases: ['abtis', 'asticker'],
  category: 'admin',
  description: 'Enable or disable auto-sticker conversion (images/videos automatically become stickers)',
  usage: '.antisticker <on/off>',
  groupOnly: true,
  adminOnly: true,
  botAdminNeeded: false,
  
  async execute(sock, msg, args, extra) {
    try {
      if (!args[0]) {
        const settings = database.getGroupSettings(extra.from);
        const status = settings.antisticker ? 'ON' : 'OFF';
        return extra.reply(
          `📌 *AutoSticker Status*\n\n` +
          `Status: *${status}*\n\n` +
          `When enabled, all images and videos sent in this group will automatically be converted to stickers.\n\n` +
          `Usage:\n` +
          `  .anisticker on\n` +
          `  .antisticker off`
        );
      }
      
      const opt = args[0].toLowerCase();
      
      if (opt === 'on') {
        if (database.getGroupSettings(extra.from).antisticker) {
          return extra.reply('*AutoSticker is already ON*');
        }
        database.updateGroupSettings(extra.from, { antisticker: true });
        return extra.reply('✅ *Antitag ON 🔛*\n\nAll images and videos will now automatically be converted to stickers!');
      }
      
      if (opt === 'off') {
        if (!database.getGroupSettings(extra.from).antisticker) {
          return extra.reply('*antisticker is already OFF*');
        }
        database.updateGroupSettings(extra.from, { autosticker: false });
        return extra.reply('❌ *Antitag OFF 📴*');
      }
      
      return extra.reply('❌ Invalid option!\nUsage: .antisticker <on/off>');
    } catch (error) {
      console.error('[antisicker Command Error]:', error);
      return extra.reply('❌ Error updating  antisticker setting.');
    }
  }
};

